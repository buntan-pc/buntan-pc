...
SPHA       |011100100000100100| stack から値とアドレスをポップし pmem_buf に書き、アドレスをプッシュ
                                pmem_buf は 2 ビットのレジスタ。
                                SPLA で指定された 16 ビット値と組み合わせて 1 つの命令ワードを構成する。
SPLA       |011100100000100101| stack から値をアドレスをポップし pmem に書き、アドレスをプッシュ
                                stack[1] = data, stack[0] = addr
...
| 011100 | 1 | 000     0     0  |    Func   |  その他の即値無し命令
...
load_sr   指定されたシステムレジスタに alu_out をロード
rst_sr    指定されたシステムレジスタをリセット
...
always @(posedge clk, posedge rst) begin
  if (rst)
    fpmin <= 16'h4000;
  else if (load_fp && alu_out < fpmin)
    fpmin <= alu_out;
  else if (rst_sr && stack0 === `SR_FPMIN)
    fpmin <= fp;
end
...
function [15:0] get_sysreg(input [15:0] sr_addr, input [15:0] fpmin);
  case (sr_addr)
    `SR_FPMIN: return fpmin;
